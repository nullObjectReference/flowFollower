public class Robot {
    ...
    public boolean followFlow(int flow){
        ...
        if (flow > 0 && op.flowField.distance < 3) { //If the robot is close enough to the flow field that it should be effected
        ...
        return Math.abs(error[0]) < 1 && Math.abs(error[1]) < 1;
    }
